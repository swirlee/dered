...
<form action="/floor/floor_floor_doedit" method="post" id="theForm">
<div class="box_floor">
  <div class="box_floor_six">
    <div class="box_floor_class">
    	<input type="hidden" name="pid" id="pid" value="${floor.pid?default('')}">
      <input type="hidden" name="id" id="id" value="${floor.id?default('')}">
      <input type="hidden" name="arr" id="arr">
      <b>楼层标题：</b>
      <input name="gf_name" type="text" id="gf_name" value="${floor.floorname?default('')}">
    </div>
    ...
    <div class="box_floor_prodel"> <em class="floor_warning">注释：最多选择10件商品，双击删除已有商品信息，按下鼠标拖动商品次序</em>
      <div class="floor_box_pls ui-sortable" id="floor_goods_info">
        ...
        <ul ondblclick="$(this).remove();" name='seengoods' goods_id="${goods.id?default('')}" class="floor_pro">
          <li class="floor_pro_img"><img src="${goods.path?default('')}" width="100" height="100"></li>
          <li class="floor_pro_name">${goods.goodsname?default('')}</li>
        </ul>
		</#list>
		</#if>
      </div>
    ...
    <div class="floor_choose_box">
     <span class="floor_choose_sp">
		...
		<div id="floor_goods_list">
			<ul class="floor_sear_pro" id="floor_sear_pro">
